"""
uspi/core/adapters/compal_adapter.py

仁宝 (Compal) ODM 适配器 / Compal ODM Adapter.

仁宝电脑是全球领先的笔记本电脑及服务器代工设计制造厂商，
为 DELL、HP、LENOVO 等品牌代工服务器及零部件。
Compal is a leading ODM for notebooks and servers,
...
class _CompalSpecParser(HTMLParser):
...
class CompalAdapter(BaseAdapter):
    """仁宝 ODM 适配器 / Compal ODM adapter.

    Attributes:
        name: 适配器英文标识 / Adapter English code.
        name_zh: 适配器中文名 / Adapter Chinese name.
        source_url: 仁宝官网 / Compal official website.
    ...
    def _parse_lookup_response(self, part_number: str, html: str) -> Optional[ServerPart]:
        """解析仁宝官网 HTML 响应 / Parse Compal official HTML response.
        ...
        return ServerPart(
            ...
            description=f"Compal ODM part {part_number}",
            description_zh=f"仁宝 ODM 零件 {part_number}",
            specifications={},
            raw_specifications=parser.specs,
            sources=[
                PriceSource(
                    source_name="Compal_Official",
                    source_name_zh="仁宝官网",
                    url=self.source_url,
    ...
    def search_by_spec(self, **specs: Any) -> List[ServerPart]:
        """按规格参数搜索仁宝零件 / Search Compal parts by specifications.
        ...
        mock_parts = [
            self._mock_lookup("CPA12345678"),
            self._mock_lookup("C87654321"),
        ]
    ...
    def _mock_lookup(self, part_number: str) -> Optional[ServerPart]:
        ...
        return ServerPart(
            ...
            description="Compal ODM part (inferred from pattern)",
