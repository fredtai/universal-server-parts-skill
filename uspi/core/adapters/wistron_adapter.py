"""
uspi/core/adapters/wistron_adapter.py

纬创 (Wistron) ODM 适配器 / Wistron ODM Adapter.

纬创资通是全球领先的信息及通讯产品代工设计制造厂商，
为 DELL、HP、LENOVO 等品牌代工服务器及零部件。
Wistron is a leading ODM for information and communication products,
producing server parts for DELL, HP, LENOVO, and others.
"""
...
class _WistronSpecParser(HTMLParser):
...
class WistronAdapter(BaseAdapter):
    """纬创 ODM 适配器 / Wistron ODM adapter.

    Attributes:
        name: 适配器英文标识 / Adapter English code.
        name_zh: 适配器中文名 / Adapter Chinese name.
        source_url: 纬创官网 / Wistron official website.
    ...
    part_prefixes = ["WIS", "W", "60"]

    def lookup(self, part_number: str) -> Optional[ServerPart]:
        """按零件号查询纬创零件信息 / Look up a Wistron part by part number.
    ...
    def _parse_lookup_response(self, part_number: str, html: str) -> Optional[ServerPart]:
        """解析纬创官网 HTML 响应 / Parse Wistron official HTML response.
        ...
        return ServerPart(
            ...
            description=f"Wistron ODM part {part_number}",
            description_zh=f"纬创 ODM 零件 {part_number}",
            specifications={},
            raw_specifications=parser.specs,
            sources=[
                PriceSource(
                    source_name="Wistron_Official",
                    source_name_zh="纬创官网",
                    url=self.source_url,
    ...
    def search_by_spec(self, **specs: Any) -> List[ServerPart]:
        """按规格参数搜索纬创零件 / Search Wistron parts by specifications.
        ...
        mock_parts = [
            self._mock_lookup("WIS12345678"),
            self._mock_lookup("W87654321"),
        ]
    ...
    def _mock_lookup(self, part_number: str) -> Optional[ServerPart]:
        ...
        return ServerPart(
            ...
            description="Wistron ODM part (inferred from pattern)",
