...
Foxconn produces server parts for DELL, HP, APPLE, and others.
"""
...
class FoxconnAdapter(BaseAdapter):
    ...
    def _parse_response(self, part_number: str, html: str) -> Optional[ServerPart]:
        """解析富士康官网 HTML 响应 / Parse Foxconn HTML response."""
        ...
        now = utc_now()
        return ServerPart(
            part_number=part_number, manufacturer="FOXCONN", manufacturer_zh=self.name_zh,
            ...
            sources=[PriceSource(
                source_name="Foxconn_Official", source_name_zh="\u5bcc\u58eb\u5eb7\u5b98\u7f51",
    ...
    def search_by_spec(self, **specs: Any) -> List[ServerPart]:
        ...
        ODM 公开规格搜索能力有限，返回空列表。
        Public ODM spec search is limited.
    ...
    def _mock_lookup(self, part_number: str) -> ServerPart:
        ...
        return make_mock_part(
            ...
            description=f"Foxconn ODM part {part_number}",
            description_zh=f"\u9e3f\u6d77/\u5bcc\u58eb\u5eb7 ODM \u96f6\u4ef6 {part_number}\uff08\u6a21\u62df\u6570\u636e\uff09",
        )
