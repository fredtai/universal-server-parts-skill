...
和硕联合科技是华硕集团分割出来的代工制造厂商，
为 ASUS、APPLE、MSI 等品牌代工服务器及零部件。
Pegatron is a spin-off from ASUS, providing manufacturing
services for ASUS, APPLE, MSI, and others.
"""
...
class _PegatronSpecParser(HTMLParser):
...
class PegatronAdapter(BaseAdapter):
    ...
    def _parse_lookup_response(self, part_number: str, html: str) -> Optional[ServerPart]:
        """解析和硕官网 HTML 响应 / Parse Pegatron official HTML response.
    ...
    def search_by_spec(self, **specs: Any) -> List[ServerPart]:
        ...
        mock_parts = [
            self._mock_lookup("PEG12345678"),
            self._mock_lookup("P87654321"),
        ]
    ...
    def _mock_lookup(self, part_number: str) -> Optional[ServerPart]:
        ...
        return ServerPart(
            ...
            description="Pegatron ODM part (inferred from pattern)",
