"""
uspi/core/adapters/quanta_adapter.py

广达 (Quanta) ODM 适配器 / Quanta ODM Adapter.

Quanta Cloud Technology (QCT) 是广达电脑旗下数据中心品牌，
为 DELL、HP、AWS 等代工服务器及云基础设施。
Quanta Cloud Technology (QCT) is Quanta Computer's data center brand,
producing server parts for DELL, HP, AWS, and others.
"""
...
class _QuantaSpecParser(HTMLParser):
...
class QuantaAdapter(BaseAdapter):
    """广达 ODM 适配器 / Quanta ODM adapter.

    Attributes:
        name: 适配器英文标识 / Adapter English code.
        name_zh: 适配器中文名 / Adapter Chinese name.
        source_url: QCT 官网 / QCT official website.
    ...
    name = "quanta"
    name_zh = "广达"
    source_url = "https://www.qct.io"
    reliability_score = 0.65
    oem_brands = ["DELL", "HP", "AWS"]
    part_prefixes = ["QCT", "Q"]

    def lookup(self, part_number: str) -> Optional[ServerPart]:
        """按零件号查询广达零件信息 / Look up a Quanta part by part number.

        先检查适配器是否启用，再尝试从 QCT 官网获取数据；
    ...
    def _parse_lookup_response(self, part_number: str, html: str) -> Optional[ServerPart]:
        """解析 QCT 官网 HTML 响应 / Parse QCT official HTML response.
        ...
        return ServerPart(
            ...
            description=f"Quanta ODM part {part_number}",
            description_zh=f"广达 ODM 零件 {part_number}",
            specifications={},
            raw_specifications=parser.specs,
            sources=[
                PriceSource(
                    source_name="Quanta_QCT",
                    source_name_zh="广达QCT官网",
                    url=self.source_url,
    ...
    def search_by_spec(self, **specs: Any) -> List[ServerPart]:
        """按规格参数搜索广达零件 / Search Quanta parts by specifications.
        ...
        mock_parts = [
            self._mock_lookup("QCT12345678"),
            self._mock_lookup("Q87654321"),
        ]
    ...
    def _mock_lookup(self, part_number: str) -> Optional[ServerPart]:
        ...
        return ServerPart(
            ...
            description="Quanta ODM part (inferred from pattern)",
